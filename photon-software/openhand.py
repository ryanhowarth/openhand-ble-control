...
class OpenHand():
	dyn = None		#Photon device
	port = None		#mounted port, /devttyUSB# in Linux, COM# in Windows
	servo_ids = []
	servos = []
	
	servo_speed = 1.0
	max_torque = 0.2	#Dynamixel suggests capping max torque at 0.4 of stall torque
	...
	def __init__(self,port,servo_ids,series="RX"):
		self.port = port
		self.dyn = Photon_Device()	#In place of usb2dynamixel device
		print 'gets here'
		...
		for servo in self.servos:
			...
			time.sleep(self.pause)
     		print("Max Torque: ", self.max_torque)
